...
* \section DeviceConfiguration_Cold  Define Application Configurations 
  ...
  - The BlueNRG-1, BlueNRG-2 controller registers values are defined on file system_BlueNRG1.c through the configuration table: 
    ...
    - At device initialization, on reset/power-on, the function SystemInit() sets the
      starting parameters defined on the COLD_START_CONFIGURATION table within
      ...
      As consequence, user application must define the following  preprocessor options, based on specific application configuration PCB:
    ...
    - Regarding the ATB0_ANA_ENG_REG, ATB1_ANA_ENG_REG registers settings, some test modes are also available in order to address some test scenarios.
      ...
      User should sets such registers as follows:
        ...
        - Internal RO time test mode: 
            ...
            - cold_start_config[2] = LS_RO_MEAS_ATB0 (0x36)
            - cold_start_config[5] = LS_RO_MEAS_ATB1 (0x34)
...
*    uint32_t savedMSP
*
*  Private storage to hold the saved Main Stack Pointer.
* ------------------------------------------------------------------------------ */
SECTION(".savedMSP")
...
/* ------------------------------------------------------------------------------
*  volatile uint32_t rfTimeout;
*
*  Timeout for the next RF transaction.
*  
* ------------------------------------------------------------------------------ */
SECTION(".rfTimeout")
REQUIRED(uint32_t volatile rfTimeout);
...
#ifndef NO_SMART_POWER_MANAGEMENT
    /* Verify if load the hot table from the application */
...
/**
 * @brief Analog test bus 0 settings for 
 * internal Low Frequency Ring Oscillator
 * measurement
 */
#define LS_RO_MEAS_ATB0             0x36
/**
 * @brief Analog test bus 1 settings for 
 * internal Low Frequency Ring Oscillator
 * measurement
 */
#define LS_RO_MEAS_ATB1             0x34
/**
...
/* Timeout Threshold used to evaluate if load the hot table at application level */
#define TIMEOUT_THRESHOLD  40
...
__STATIC_INLINE void nextRF_transaction(uint16_t reset_reason)
{
  ...
  if (time_diff > 0) {
    ...
    /* Nothing to do, too close to RF timeout. Wait the radio transaction */
  }
